...
left join
	shopee_order_db__order_v2_tab or
on
	oi.orderid = or.orderid
where 
	from_unixtime(create_time) between "2017-08-01" and "2017-08-31"
...
q11 = '''
select t1.userid, t1.orderid, t1.checkoutid, t1.itemid, t1.item_price, t1.amount, time
	from table1 t1
left join
	shopee_order_details_db__order_details_tab d
on 
	t1.userid = d.userid
where
	from_unixtime(create_time) between "2017-08-01" and "2017-08-31"
...
group by
	grass_region
'''
spark.sql(q52).to.Pandas().to_csv("total_numOfOrder_otherPurchase.csv")
...
q61 = '''
select distinct userid, substring(time,0,10) as date1,rank as rank1
from table42
where rank = 1
'''
...
q62 = '''
select distinct userid, substring(time,0,10) as date2,rank as rank2
from table42
where rank = 2
...
q63 = '''
select avg (datediff(date2,date1)) as avg_datediff
from table61 t61
